(function(Scratch) {
  ...
  class SpriteParenting {
      ...
      parent_sprites(args, util) {
        let target1 = Scratch.Cast.toString(args.ONE) == 'myself' ? util.target.id: 'idk lol';
        let target2 = Scratch.Cast.toString(args.TWO) == 'myself' ? util.target.id : 'idk lol';
        target1['parentedTo'] = target2
        target2['children'].push(target1)
      }
      unparent_sprites(args, util) {    
        let target1 = Scratch.Cast.toString(args.ONE) == 'myself' ? util.target.id : 'idk lol';
        let target2 = Scratch.Cast.toString(args.TWO) == 'myself' ? util.target.id : 'idk lol';
        //todo: remove the stuff lol
      }
      unparent_all(args, util) {    
        let target1 = Scratch.Cast.toString(args.ONE) == 'myself' ? util.target.id : 'idk lol';
